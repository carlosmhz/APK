...
function entrar() {
  ...
  if (user.nombre === 'Carlos') {
    ...
    document.getElementById('btnPedirWrap').style.display = 'block';
  }
}
...
function mostrarFormSolicitud() {
  var sec = document.getElementById('solLuccianaSection');
...
function escucharSolicitudes() {
  db.ref('solicitudes').orderByChild('estado').equalTo('pendiente').on('value', function(snap) {
    ...
    if (items.length > 0) document.getElementById('solSection').style.display = 'block';
    else document.getElementById('solSection').style.display = 'none';
    var badge = document.getElementById('solBadge');
    if (items.length > 0) { badge.textContent = items.length + ' pendientes'; badge.style.display = 'inline'; }
