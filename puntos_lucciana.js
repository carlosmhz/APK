...
function renderSolicitudes(items) {
  ...
  items.forEach(function(item) {
    ...
    div.innerHTML =
      '<div class="sol-motivo">' + v.motivo + '</div>' +
      '<div class="sol-meta">' + fechaStr + ' · <a href="' + v.foto + '" target="_blank" style="color:var(--gold);text-decoration:none;">&#x1F4F7; Ver foto</a></div>' +
      '<div class="sol-btns">' +
        '<input type="number" class="sol-pts" placeholder="pts" min="1" id="pts_' + item.key + '">' +
        '<button class="sol-aprobar" onclick="aprobarSolicitud(\'' + item.key + '\',\'' + v.motivo.replace(/'/g,'') + \'\')">✔ Aprobar</button>' +
        '<button class="sol-rechazar" onclick="rechazarSolicitud(\'' + item.key + '\')">&#x2716; Rechazar</button>' +
      '</div>';
    card.appendChild(div);
  });
}

function aprobarSolicitud(key, motivo) {
