...
function entrar() {
  ...
  iniciarEscucha();
  var tabSol = document.getElementById('tabSol');
  if (user.nombre === 'Carlos') {
    tabSol.style.display = 'block';
    tabSol.childNodes[0].textContent = 'Solicitudes ';
    escucharSolicitudes();
  } else if (user.nombre === 'Lucciana') {
    tabSol.style.display = 'block';
    tabSol.childNodes[0].textContent = 'Pedir puntos';
  } else {
    tabSol.style.display = 'none';
  }
}
...
function notificarLucciana(cambio, nuevo, motivo) {
  ...
  var emoji = cambio > 0 ? '🌟' : '😬';
  var lineas = [
    emoji + ' Lucciana, ' + usuarioActual.nombre + ' te ha ' + (cambio > 0 ? 'sumado ' : 'restado ') + signo + cambio + ' puntos por: ' + motivo + '.',
    'Tu puntuacion actual es ' + (nuevo > 0 ? '+' : '') + nuevo + ' puntos.',
    '',
    'Ver tu puntuacion: https://tinyurl.com/Puntos-Lucciana'
  ];
  pendingWAmsg = lineas.join('\n');
...
// -- SOLICITUDES --
function previewSolFoto(input) {
  var img = document.getElementById('solFotoPreview');
...
function enviarSolicitud() {
  ...
  var foto   = document.getElementById('solFoto').files[0];
  if (!motivo) { mostrarToast('Escribe la tarea que has hecho'); return; }
  if (!foto)   { mostrarToast('La foto es obligatoria'); return; }
  document.getElementById('loaderOverlay').style.display = 'flex';
  var formData = new FormData();
  ...
  fetch(UPLOAD_URL, { method: 'POST', body: formData })
    ...
    .then(function(data) {
      document.getElementById('loaderOverlay').style.display = 'none';
      if (!data.ok) { mostrarToast('Error al subir la foto'); return; }
      db.ref('solicitudes').push({ ts: Date.now(), motivo: motivo, foto: data.url, estado: 'pendiente' });
      document.getElementById('solMotivo').value = '';
      document.getElementById('solFoto').value = '';
      document.getElementById('solFotoPreview').style.display = 'none';
      mostrarToast('Solicitud enviada!');
      notificarCarlos(motivo);
    })
    .catch(function() {
      document.getElementById('loaderOverlay').style.display = 'none';
      mostrarToast('Error al enviar la solicitud');
    });
}

function notificarCarlos(motivo) {
  var lineas = [
    '📋 Lucciana ha solicitado puntos por: ' + motivo + '.',
    'Entra en la app para revisarlo:',
    'https://tinyurl.com/Puntos-Lucciana'
  ];
  var msg = lineas.join('\n');
  var a = document.createElement('a');
  a.href = 'https://wa.me/34618234617?text=' + encodeURIComponent(msg);
  a.target = '_blank'; a.rel = 'noopener';
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
}

function escucharSolicitudes() {
  db.ref('solicitudes').orderByChild('estado').equalTo('pendiente').on('value', function(snap) {
    var items = [];
    snap.forEach(function(child) { items.push({ key: child.key, val: child.val() }); });
    items.reverse();
    renderSolicitudes(items);
    var badge = document.getElementById('solBadge');
...
function renderSolicitudes(items) {
  var card = document.getElementById('solCard');
  var empty = document.getElementById('solEmpty');
  card.querySelectorAll('.sol-item').forEach(function(el){ el.remove(); });
  if (!items || items.length === 0) { empty.style.display = 'block'; return; }
  empty.style.display = 'none';
  items.forEach(function(item) {
    var v = item.val;
    var fecha = new Date(v.ts);
    var fechaStr = fecha.toLocaleDateString('es-ES',{day:'2-digit',month:'2-digit'}) + ' ' + fecha.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'});
    var motivoSafe = v.motivo.replace(/"/g, '');
    var div = document.createElement('div');
    div.className = 'sol-item';
    var html = '';
    html += '<div class="sol-motivo">' + v.motivo + '</div>';
    html += '<div class="sol-meta">' + fechaStr + ' · <a href="' + v.foto + '" target="_blank" style="color:var(--gold);text-decoration:none;">&#x1F4F7; Ver foto</a></div>';
    html += '<div class="sol-btns">';
    html += '<input type="number" class="sol-pts" placeholder="pts" min="1" id="pts_' + item.key + '">';
    html += '<button class="sol-aprobar" onclick="aprobarSolicitud(this)" data-key="' + item.key + '" data-motivo="' + motivoSafe + '">✔ Aprobar</button>';
    html += '<button class="sol-rechazar" onclick="rechazarSolicitud(this)" data-key="' + item.key + '">✖ Rechazar</button>';
    html += '</div>';
    div.innerHTML = html;
    card.appendChild(div);
  });
...
function aprobarSolicitud(btn) {
  var key    = btn.getAttribute('data-key');
  var motivo = btn.getAttribute('data-motivo');
  var pts = parseInt(document.getElementById('pts_' + key).value);
  if (!pts || pts <= 0) { mostrarToast('Introduce los puntos a dar'); return; }
  var nuevo = scoreActual + pts;
  db.ref('puntos').set(nuevo);
  db.ref('historial').push({ ts: Date.now(), usuario: usuarioActual.nombre, motivo: 'Solicitud: ' + motivo, cambio: pts, total: nuevo });
  db.ref('solicitudes/' + key).update({ estado: 'aprobada' });
  notificarLucciana(pts, nuevo, 'Solicitud aprobada: ' + motivo);
  mostrarToast('+' + pts + ' puntos asignados');
}
...
function verTab(tab) {
  ...
  document.getElementById('tabSol').className    = 'tab' + (tab==='sol'    ? ' on' : '');
  ...
  document.getElementById('solLuccianaSection').style.display = 'none';
  ...
  if (tab==='sol') {
    if (usuarioActual && usuarioActual.nombre === 'Lucciana') {
      document.getElementById('solLuccianaSection').style.display = 'block';
    } else {
