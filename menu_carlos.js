...
function xor(bytes, pin) {
  var out = [];
  for (var i = 0; i < bytes.length; i++) {
    out.push(bytes[i] ^ pin.charCodeAt(i % pin.length));
  }
  return out;
}
...
function toHex(str) {
  ...
  var xored = xor(bytes, '____PIN____');
  return xored.map(function(b){ return ('0'+b.toString(16)).slice(-2).toUpperCase(); }).join('');
}

function toHexWithPin(str, pin) {
  var bytes = [];
  for (var i = 0; i < str.length; i++) {
    var c = str.charCodeAt(i);
    if (c < 128) {
  ...
  var xored = xor(bytes, pin);
  return xored.map(function(b){ return ('0'+b.toString(16)).slice(-2).toUpperCase(); }).join('');
}

function fromHexWithPin(h, pin) {
  var bytes = [];
  ...
  while (i < xored.length) {
    ...
    if (b < 128) {
      ...
      str += String.fromCharCode(((b & 31) << 6) | (xored[i+1] & 63)); i += 2;
    } else {
      str += String.fromCharCode(((b & 15) << 12) | ((xored[i+1] & 63) << 6) | (xored[i+2] & 63)); i += 3;
    }
  }
  return str;
}
...
function generar() {
  ...
  var cifrado = toHexWithPin('OK:' + ri, pin);
...
function desbloquear() {
  ...
  try {
    var descifrado = fromHexWithPin(payload.r, pin);
